...
class EMPlaneWaveInLossyMedium(ThreeDScene):
    def construct(self):
        ...
        # Constants for the wave in a lossy medium
        alpha = 0.3  # Attenuation constant
        beta = 1.0   # Phase constant
        omega = 2 * PI  # Angular frequency

        # Define the electric field function (E-field) with attenuation
        ...
        # Define the magnetic field function (H-field) with attenuation
        ...
        e_wave = always_redraw(lambda: 
            VGroup(*[Arrow3D(
                ...
                color=YELLOW
            ) for z in np.arange(-5, 5, 1.0)])  # Increased step size to 1.0 for performance
        )
